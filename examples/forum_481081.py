...
setups['side_balanced_colonels'] = setups['board'].apply(lambda x: x.side_sums(colonel).max() == 1)
...
setups['lane_balanced_majors'] = setups['board'].apply(lambda x: x.lane_sums(major).max() == 1)
...
setups['row_balanced_majors'] = setups['board'].apply(lambda x: x.row_sums(major).max() == 1)
